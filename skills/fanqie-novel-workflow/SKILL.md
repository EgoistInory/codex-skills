...
将目标书籍根目录的 `感情与成人内容规范.md` 作为该模块的明确入口，不要把文件名改写成泛化概念，也不要仅凭关键词猜测其他文件。若其他项目使用不同文件名，只有项目 `AGENTS.md`、书籍简介或交接文件明确给出等价路径时才按其入口替换。

不要把该文件设为所有章节的固定上下文。结合当前章纲、近期正文、状态文件、交接记录和本次任务判断是否触发；判断看场景实际功能，不做词语命中式路由。

出现以下任一场景功能时，在动笔、扩写、重写或审校前读取该文件：
...
不承担这些功能的行动、探索、成长、日常或制度推进章节，无需重复加载。触发后，以该文件定义的内容尺度、人物边界、题材术语、呈现重点和审校标准为准；本 Skill 不另行发明或覆盖某本书的具体规则。

若剧情明确触发，但目标路径不存在，先检查项目入口是否声明了等价文件；没有明确入口时，不自行创建文件或用相似名称猜测，向用户说明缺少项目专项边界，或只推进不依赖该边界的部分。
...
2. 对照章纲和设定，列出本章必须推进、禁止提前揭露、禁止改写的内容；同时判断是否触发 `感情与成人内容规范.md`，触发时先读取该文件再设计场景。
...
36. 若本章触发 `感情与成人内容规范.md`，是否已读取并按该文件核对人物边界、内容尺度、关系后果和呈现重点；是否误用本 Skill 的泛化描述覆盖了目标书籍自己的规则或术语。
